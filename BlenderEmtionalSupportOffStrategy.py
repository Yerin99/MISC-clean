...
class BlenderbotSmallForConditionalGenerationOffStrategy(BlenderbotSmallForConditionalGeneration):
    """전략 주입을 끄기 위한 래퍼 모델.

    - encoder의 `strategy_embedding` 가중치를 매 forward 전에 0으로 세팅하여
      디코더의 전략 크로스 어텐션에 전달되는 전략 메모리를 사실상 무력화합니다.
    - 나머지 로직(로짓/손실 계산, COMET 경로, 표준 HF 인터페이스)은 그대로 유지됩니다.
    """

    def forward(self, *args, **kwargs):
        enc = getattr(self, 'model', None)
        if enc is not None and hasattr(enc, 'encoder') and hasattr(enc.encoder, 'strategy_embedding'):
            with torch.no_grad():
                enc.encoder.strategy_embedding.weight.zero_()
...
class Args():
   def __init__(self):
    ...
    #    self.model_name_or_path = './blender-small'
       ...
       self.data_cache_dir = './cached_off_strategy'
       self.block_size = 512
       self.do_train = True
       self.do_eval = False
...
def main(args):
    ...
    if getattr(args, 'use_strategy_attn', True):
        model = BlenderbotSmallForConditionalGeneration.from_pretrained(args.model_name_or_path, cache_dir=args.model_cache_dir)
    else:
        model = BlenderbotSmallForConditionalGenerationOffStrategy.from_pretrained(args.model_name_or_path, cache_dir=args.model_cache_dir)
    ...
    if args.do_train:
        ...
        if getattr(args, 'use_strategy_attn', True):
            ...
            model = BlenderbotSmallForConditionalGenerationOffStrategy.from_pretrained(args.output_dir, from_tf=False)
        model.to(args.device)
